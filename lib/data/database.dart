...
class Hotels extends Table {
  ...
  TextColumn get checkInHour => text().withDefault(const Constant('14:00'))(); // New v5
  TextColumn get checkOutHour => text().withDefault(const Constant('11:00'))(); // New v5
  RealColumn get defaultRoomPrice => real().withDefault(const Constant(0.0))(); // New v5
...
class Rooms extends Table {
  TextColumn get id => text()();
  TextColumn get hotelId => text().references(Hotels, #id)();
  TextColumn get roomTypeId => text().nullable().references(RoomTypes, #id)(); // New v6
  TextColumn get name => text()();
  IntColumn get capacity => integer().withDefault(const Constant(2))();
  TextColumn get status => text().withDefault(const Constant('clean'))(); // clean, dirty, maintenance
...
class Guests extends Table {
  ...
  BoolColumn get isBanned => boolean().withDefault(const Constant(false))(); // New
...
class Bookings extends Table {
  ...
  TextColumn get source => text().withDefault(const Constant('direct'))(); // New: direct, airbnb, booking_com
...
class Payments extends Table { // New Table
  TextColumn get id => text()();
...
class Expenses extends Table {
  ...
  RealColumn get amount => real()();
  DateTimeColumn get date => dateTime()();
  TextColumn get category => text().withDefault(const Constant('Other'))(); // Maintenance, Supplies, Utilities, Other
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
...
class Users extends Table {
  ...
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
  TextColumn get role => text().withDefault(const Constant('owner'))(); // owner, staff
  TextColumn get hotelId => text().nullable().references(Hotels, #id)();
  TextColumn get plan => text().withDefault(const Constant('free'))(); // free, premium, premium_plus
...
class MessageTemplates extends Table {
  TextColumn get id => text()();
  TextColumn get hotelId => text().nullable().references(Hotels, #id)(); // null = global
...
@DriftDatabase(tables: [Hotels, RoomTypes, Rooms, Guests, Bookings, Payments, Expenses, Users, MessageTemplates, MessagesLog, GuestDocuments])
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (Migrator m, int from, int to) async {
      ...
      if (from < 3) {
        // Migration to v3
        try { await m.addColumn(hotels, hotels.currency); } catch (_) {}
        try { await m.addColumn(hotels, hotels.updatedAt); } catch (_) {}
        
        try { await m.addColumn(rooms, rooms.sortOrder); } catch (_) {}
        try { await m.addColumn(rooms, rooms.updatedAt); } catch (_) {}
        
        try { await m.addColumn(guests, guests.isBanned); } catch (_) {}
        try { await m.addColumn(guests, guests.updatedAt); } catch (_) {}
        
        try { await m.addColumn(bookings, bookings.source); } catch (_) {}
        try { await m.addColumn(bookings, bookings.updatedAt); } catch (_) {}
        
        try { await m.createTable(payments); } catch (_) {}
      }
      if (from < 4) {
        // Migration to v4
        try { await m.addColumn(rooms, rooms.status); } catch (_) {}
      }
      if (from < 5) {
        // Migration to v5
        try { await m.addColumn(hotels, hotels.checkInHour); } catch (_) {}
        try { await m.addColumn(hotels, hotels.checkOutHour); } catch (_) {}
        try { await m.addColumn(hotels, hotels.defaultRoomPrice); } catch (_) {}
      }
      if (from < 6) {
        // Migration to v6
        await m.createTable(roomTypes);
        try { await m.addColumn(rooms, rooms.roomTypeId); } catch (_) {}
      ...
      if (from < 9) {
        // Migration to v9
        try { await m.addColumn(bookings, bookings.notes); } catch (_) {}
      }
    },
  );
